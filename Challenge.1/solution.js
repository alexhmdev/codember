...
*/
const input = `usr:@midudev eme:mi@gmail.com psw:123456 age:38 loc:bcn fll:82
...
const validUserProps = ['usr:', 'eme:', 'psw:', 'age:', 'loc:', 'fll:'];
/**
 *@param {string} users
 */
function filterValidUsers(users) {
  ...
  let valid = usersArray.filter((user) => {
    let validProps = 0;
    validUserProps.forEach((prop) => {
      if (user.includes(prop)) {
        validProps++;
      }
    });
    if (validProps === validUserProps.length) {
      return user;
    }
  });
  return {
    total: valid.length,
    lastUser: valid[valid.length - 1],
  };
}

console.log(filterValidUsers(input));
